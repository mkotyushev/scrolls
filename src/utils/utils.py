...
class Agg2dWrapper(nn.Module):
    """Wrapper to predict for each Z slice and aggregate the results."""
    def __init__(self, model, full_size, size, step=1, scale_factor=None):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        if x.ndim == 4:
            assert x.shape[1] == 1, 'Single class prediction expected'
            x = x.squeeze(1)

        # Possibly not equal to (H, W) because some models
        # could do upsampling in the last layer and some not 
        H_, W_ = x.shape[1:]

        # (B * self.n_preds, H_, W_) -> (B, self.n_preds, H_, W_)
        x = x.view(B, self.n_preds, H_, W_)
        # (B, self.n_preds, H_, W_) -> (B, H_, W_)
        x = self.cnn(x)

        # Upsample to original size:
        # if upsampling is used, remove the channel dimension
        # after upsampling, otherwise just remove the channel dimension
        if isinstance(self.upsampling, nn.Identity):
            x = x.squeeze(1)
        else:
            x = self.upsampling(x)
            x = x.squeeze(1)
